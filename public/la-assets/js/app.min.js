...
$(document).ready(function() {
    function showLAFM(type, selector) {
        ...
        $(".fm_folder_sel.selected").removeClass("selected");
        $(".fm_folder_sel[folder="+fmFolder+"]").addClass("selected");
        
        $("#fm").modal('show');
        
        loadFMFiles();
    }
    function getLI(folder_name, filename) {
    ...
    function loadFMFiles() {
        ...
        $.ajax({
            dataType: 'json',
            url: $('body').attr("bsurl")+"/la/folder_files/"+fmFolder,
            success: function ( json ) {
                ...
                for (var index = 0; index < json.files.length; index++) {
                    ...
                    var li = getLI(json.folder_name, element);
                    $(".fm_file_selector ul").append(li);
                }
            }
        });
    }
    $(".fm_folder_sel").on("click", function() {
        fmFolder = $(this).attr("folder");
        $(".fm_folder_sel.selected").removeClass("selected");
            $(".fm_folder_sel[folder="+fmFolder+"]").addClass("selected");
        loadFMFiles();
    });
    ...
    $("#fm input[type=search]").keyup(function () {
        ...
        if(sstring != "") {
            ...
            for (var index = 0; index < cntFiles.length; index++) {
                ...
                if(element.includes(sstring)) {
                    var li = getLI(fmFolder, element);
                    $(".fm_file_selector ul").append(li);
                }
            }
        } else {
    ...
    $(".input-group-addon.file").on("click", function() {
        console.log(".input-group-addon.file");
        showLAFM("input", $(this).attr("selecter"));
    });
    ...
    $("body").on("click", ".fm_file_sel", function() {
        ...
        value = $(this).attr("fpath");
        if(type == "input") {
            $("input[name="+$("#image_selecter_origin").val()+"]").val(value);
            $("input[name="+$("#image_selecter_origin").val()+"]").next(".preview").css({
                "display": "block",
                "background-image": "url('"+bsurl+"/"+value+"')",
                "background-size": "cover"
            });
        }
        $("#fm").modal('hide');
